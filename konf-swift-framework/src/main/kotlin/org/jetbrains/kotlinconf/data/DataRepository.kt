...
import io.ktor.common.client.runSuspend
...
class DataRepository(val uuid: String) {
    ...
    fun findSortedSpeakers(session: Session): List<Speaker> {
    ...
    fun findRoom(session: Session): Room? {
        return org.jetbrains.kotlinconf.data.AppContext.allData?.rooms?.firstOrNull { it.id == session.roomId }
    }

    fun findCategoryItems(session: Session): List<CategoryItem> {
    ...
    companion object {
        ...
        fun registerUser(uuid: String, onComplete: (Boolean) -> Unit) {
            runSuspend {
                val created = KotlinConfApi.createUser(uuid)
                onComplete(created)
            }
        }
    }
